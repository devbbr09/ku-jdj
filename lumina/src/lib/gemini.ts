...
export interface MakeupAnalysisResult {
  overallScore: number;
  subScores: {
    eyeMakeup?: Record<string, number>;
    baseMakeup?: Record<string, number>;
    lipMakeup?: Record<string, number>;
  };
  feedback: string;
  improvements?: string[];
  strengths?: string[];
...
평가 기준:
⚠️ 중요: 각 항목을 1-10점으로 정확히 평가하세요. 5점 이하는 심각한 문제, 6-7점은 보통, 8-9점은 좋음, 10점은 완벽을 의미합니다.
...
- 색상 조화: 아이섀도우의 톤(Vision AI 분석)이 전체 메이크업 컨셉과 조화로운지 평가하세요. 레퍼런스 사진의 색상 팔레트와 얼마나 유사한지 판단하세요.
  ...
  점수 기준:
    - 1-3점: 색상이 피부톤과 전혀 맞지 않거나 과도하게 튀는 색상 사용
    - 4-5점: 색상이 어울리지 않거나 레퍼런스와 전혀 다른 색상 팔레트
    - 6-7점: 기본적인 색상 조화는 있으나 레퍼런스와 차이가 있음
    - 8-9점: 색상이 잘 어울리고 레퍼런스와 유사한 색상 팔레트
    - 10점: 완벽한 색상 조화, 레퍼런스와 거의 동일한 색상 팔레트
  Evaluation:
...
- 블렌딩 & 음영: 아이섀도의 색상 경계가 뚜렷하지 않고 부드러운 그라데이션을 이루었는지 평가하세요.
  ...
  점수 기준:
    - 1-3점: 색상 경계가 매우 뚜렷하고 블렌딩이 전혀 되지 않음
    - 4-5점: 색상 경계가 보이고 블렌딩이 부족함
    - 6-7점: 기본적인 블렌딩은 있으나 완벽하지 않음
    - 8-9점: 부드러운 그라데이션과 자연스러운 블렌딩
    - 10점: 완벽한 블렌딩, 경계선이 전혀 보이지 않음
  Evaluation:
...
1. 피부톤 매칭:
  ...
  - 자연스러움: 지나치게 밝거나 어둡지 않은 자연스러운 톤 매칭을 평가하세요.
   ...
   점수 기준:
     - 1-3점: 얼굴과 목 색상 차이가 매우 크고 부자연스러움
     - 4-5점: 피부톤이 맞지 않거나 지나치게 밝거나 어두움
     - 6-7점: 기본적인 톤 매칭은 있으나 완벽하지 않음
     - 8-9점: 자연스러운 피부톤 매칭, 얼굴-목 조화 양호
     - 10점: 완벽한 피부톤 매칭, 얼굴-목이 하나로 이어짐
- Evaluation:
...
- Data: Vision API dominantColors (립 영역), skinToneHarmony score, brightness
  점수 기준:
    - 1-3점: 립 컬러가 피부톤과 전혀 맞지 않거나 과도하게 튀는 색상
    - 4-5점: 립 컬러가 어울리지 않거나 전체 메이크업과 조화되지 않음
    - 6-7점: 기본적인 색상 조화는 있으나 완벽하지 않음
    - 8-9점: 립 컬러가 피부톤과 잘 어울리고 전체 조화 양호
    - 10점: 완벽한 립 컬러 조화, 전체 메이크업과 완벽한 조화
- Evaluation:
...
3.  **subScores의 점수를 바탕으로, 각 항목별로 가장 높은 점수를 받은 항목을 칭찬하고 가장 낮은 점수를 받은 항목을 개선점으로 서술하세요.**
4.  **분석 타입에 따라 해당 영역만 집중 분석하세요:**
    - eye: 아이섀도우, 아이라이너, 마스카라, 아이브로우, 글리터만 분석
    - base: 피부톤 매칭, 잡티 커버리지, 피부 질감, 쉐딩 & 컨투어링만 분석  
    - lip: 립 컬러와 조화, 발색과 적용, 입술 컨디션과 지속력만 분석
    - overall: 모든 영역을 균형있게 분석
5.  **민낯/메이크업/레퍼런스** 사진을 비교하며 전후 차이점과 목표 달성도를 분석하세요.
6.  **절대 다른 메이크업 영역을 언급하지 마세요.** 각 분석 타입에 해당하는 영역만 분석하세요.
7.  실제로 적용 가능한 실용적인 조언을 포함하세요.
8.  모든 내용은 전문적이면서 친근한 톤의 **한국어**로 작성하세요.
9.  **점수 평가 중요사항:**
    - overallScore는 60~100 사이의 정수
    - subScores의 각 항목은 1~10점 사이의 정수
    - **절대 모든 항목을 비슷한 점수(7-8점)로 주지 마세요**
    - **실제 품질에 따라 1-10점 사이에서 차별화된 점수를 주세요**
    - **완벽한 항목은 9-10점, 문제가 있는 항목은 1-5점을 주세요**
10.  **Vision AI 분석 결과를 적극 참고**하여 더 정확하고 객관적인 평가를 수행하세요.
11. 피드백은 최소 3줄 이상 작성하고, 해당 분석 타입에만 집중하여 작성하세요.
`;
...
export async function generateMakeupAnalysis(prompt: MakeupAnalysisPrompt): Promise<MakeupAnalysisResult> {
  try {
    ...
    const analysisTypePrompts = {
      eye: `
⚠️ 아이 메이크업 전용 분석입니다. 다른 메이크업 영역은 절대 언급하지 마세요.

분석 범위:
- 아이섀도우: 색상 조화, 블렌딩, 균일성, 입체감, 좌우 대칭성
- 아이라이너: 균일성, 좌우 대칭성, 선의 매끄러움
- 마스카라: 균일성, 좌우 대칭성, 번짐 여부
- 아이브로우: 좌우 대칭성, 채움 균일성
- 글리터: 분포 균형, 반짝임 강도, 입자 균일성

피드백 작성 규칙:
1. 아이 메이크업 관련 내용만 작성
2. 베이스 메이크업, 립 메이크업은 절대 언급하지 마세요
3. 아이섀도, 아이라이너, 마스카라, 아이브로우, 글리터만 분석
4. "아이 메이크업이..."로 시작하는 문장으로 작성
      `,
      base: `
⚠️ 베이스 메이크업 전용 분석입니다. 다른 메이크업 영역은 절대 언급하지 마세요.

분석 범위:
- 피부톤 매칭: 파운데이션 톤, 자연스러움, 얼굴-목 조화
- 잡티 커버리지: 커버 효과, 컨실러 블렌딩
- 피부 질감: 질감 표현, 균일한 발색
- 쉐딩 & 컨투어링: 쉐딩 강도, 블렌딩, 얼굴 조화

피드백 작성 규칙:
1. 베이스 메이크업 관련 내용만 작성
2. 아이 메이크업, 립 메이크업은 절대 언급하지 마세요
3. 파운데이션, 컨실러, 컨투어링, 피부톤만 분석
4. "베이스 메이크업이..."로 시작하는 문장으로 작성
      `,
      lip: `
⚠️ 립 메이크업 전용 분석입니다. 다른 메이크업 영역은 절대 언급하지 마세요.

분석 범위:
- 립 컬러와 조화: 피부톤 매칭, 전체 메이크업 조화, 명도 균형, 대비 적절성
- 발색과 적용: 색상 균일성, 경계선 선명도, 그라데이션 품질, 질감 일관성
- 입술 컨디션과 지속력: 수분감, 질감 표현, 각질 관리, 전반적 컨디션

피드백 작성 규칙:
1. 립 메이크업 관련 내용만 작성
2. 아이 메이크업, 베이스 메이크업은 절대 언급하지 마세요
3. 립스틱, 립라이너, 립글로스만 분석
4. "립 메이크업이..."로 시작하는 문장으로 작성
      `,
      overall: `
⚠️ 전체 메이크업 종합 분석입니다. 모든 영역을 균형있게 언급하세요.

분석 범위:
- 아이 메이크업: 아이섀도, 아이라이너, 마스카라, 아이브로우
- 베이스 메이크업: 파운데이션, 컨실러, 컨투어링
- 립 메이크업: 립스틱, 립라이너
- 전체 조화: 각 영역 간의 조화, 레퍼런스 유사도

피드백 작성 규칙:
1. 아이, 베이스, 립 메이크업을 모두 균형있게 언급
2. 각 영역별로 1-2문장씩 작성
3. 전체적인 조화와 완성도 중심으로 분석
4. "전체적으로..."로 시작하는 문장으로 작성
      `
    };
    ...
    const userPrompt = `
      ...
      🔥 중요: '${prompt.analysisType}' 메이크업 전용 분석입니다.
      
      ${analysisTypePrompts[prompt.analysisType]}
      
      📋 분석 요구사항:
      1. ${prompt.analysisType} 메이크업만 분석하세요
      2. 다른 메이크업 영역은 절대 언급하지 마세요
      3. 피드백은 해당 영역에만 집중하여 작성하세요
      4. JSON 형식으로 정확한 점수와 피드백을 제공하세요
      
      🎯 점수 평가 기준 (${prompt.analysisType}):
      ${prompt.analysisType === 'eye' ? `
      - 아이섀도 블렌딩(30%): 색상 조화, 블렌딩 품질, 균일성
      - 아이라이너 정확성(25%): 선의 매끄러움, 좌우 대칭성
      - 마스카라 균일성(25%): 뭉침 없음, 좌우 대칭성, 번짐 없음
      - 아이브로우 대칭성(20%): 좌우 균형, 채움 균일성` : ''}
      ${prompt.analysisType === 'base' ? `
      - 피부톤 매칭(40%): 얼굴-목 조화, 자연스러움
      - 잡티 커버리지(30%): 커버 효과, 블렌딩 품질
      - 컨투어링 자연스러움(30%): 쉐딩 강도, 블렌딩, 얼굴 조화` : ''}
      ${prompt.analysisType === 'lip' ? `
      - 립 컬러 조화(40%): 피부톤 매칭, 전체 조화
      - 립 라인 정확성(30%): 경계선 선명도, 그라데이션 품질
      - 블렌딩 완성도(30%): 색상 균일성, 질감 일관성` : ''}
      ${prompt.analysisType === 'overall' ? `
      - 전체 조화(40%): 아이-베이스-립 간의 조화
      - 기술적 완성도(30%): 각 영역의 기술적 품질
      - 레퍼런스 유사도(30%): 목표 스타일 달성도` : ''}
    `;
